...
if(isset($_GET['hapus'])) {
    ...
    mysqli_query($conn, "DELETE FROM properti WHERE id = '$id_hapus'");
    echo "<script>alert('Properti berhasil dihapus'); window.location='kelola_properti.php';</script>";
}

$query = mysqli_query($conn, "SELECT properti.*, users.nama as nama_owner 
                              FROM properti 
                              JOIN users ON properti.owner_id = users.id 
                              ORDER BY properti.id DESC");
?>
...
<head>
    ...
    <div class="main-wrapper">
        <header class="topbar">
            <div class="user-info" style="font-weight: bold; font-size: 18px; color: var(--text-dark);">Data Seluruh Properti</div>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php 
                                ...
                                $warna_badge = ($status == 'TERSEDIA') ? 'background: #eef9f1; color: #27ae60;' : 'background: #fdf2f2; color: #e74c3c;';
                            ?>
                        <span class="badge" style="<?php echo $warna_badge; ?>">
                            <?php echo $status; ?>
                        </span>
                     </td>
                        <td>
                          <a href="detail_properti_admin.php?id=<?php echo $row['id']; ?>" class="btn-action btn-detail">Detail</a>
                          <a href="kelola_properti.php?hapus=<?php echo $row['id']; ?>" class="btn-action btn-del" onclick="return confirm('Hapus properti ini?')">Hapus</a>
                        </td>
                   </tr>
                        <?php endwhile; ?>
                    </tbody>
